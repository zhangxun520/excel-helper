...
public class Money implements Serializable, Comparable<Object> {
	...
	/**
	 * 获取分级后的金额数
	 * @param scale 金额等级 如10,1000,10000等
	 * @param scale 保留小数位数
	 * @return 分级后的金额数，单位视传递的参数不同，基准单位为元
	 */
	public BigDecimal getScaleAmount(int divideFactor, int scale) {
		return getAmount().divide(BigDecimal.valueOf(divideFactor), scale, BigDecimal.ROUND_HALF_EVEN);
	}

	/**
	 * 获取分级后的金额数,，小数位保留到小数点后6位
	 * @param int 除数 如10,1000,10000等
	 * @return 分级后的金额数，单位视传递的参数不同，基准单位为元
	 */
	public BigDecimal getScaleAmount(int divideFactor) {
		return getScaleAmount(divideFactor, 6);
	}
